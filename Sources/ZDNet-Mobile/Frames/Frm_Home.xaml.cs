...
namespace ZDNet.Frames
	...
	public partial class Frm_Home : PhoneApplicationPage, IPhoneApplicationPage
		...
		class WebNavigator
			...
			public bool IsLast ( object Source )
				...
				if ( Source != null && this.Items.Count > 0 )
					...
					//-----------------------------------------------------------------------------------------------------------
					if ( Source is string && LastItem.Source is string )
						{
						//-------------------------------------------------------------------------------------------------------
						string A = (string)Source;
						string B = (string)LastItem.Source;
		...
		private ApplicationBarIconButton OptionsAppBarButton          = null; // Page Section
		//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		...
		public Frm_Home ()
			...
			this.OptionsAppBarButton          = new ApplicationBarIconButton ();
			...
			this.OptionsAppBarButton         .IconUri = new Uri ( "/Images/Application/options.png"     , UriKind.Relative );
			...
			this.OptionsAppBarButton         .Text = "paramètres";
			...
			this.OptionsAppBarButton         .Click += this.OnOptionsButtonClick;
		...
		private void SetTitle ( SectionType Section )
			...
			switch ( Section )
				{
				//---------------------------------------------------------------------------------------------------------------
				case SectionType.All       : this.Section.Text = "DERNIERS ARTICLES"; break;
				case SectionType.Bookmarks : this.Section.Text = "MES FAVORIS";       break;
				case SectionType.Search    : this.Section.Text = "RECHERCHE";         break;
				//---------------------------------------------------------------------------------------------------------------
				}
			//-------------------------------------------------------------------------------------------------------------------
			}
		...
		protected override void OnBackKeyPress ( System.ComponentModel.CancelEventArgs Args )
			...
			if ( ! Args.Cancel )
				...
				// Gestion du Buffer
				//---------------------------------------------------------------------------------------------------------------
				if ( this.PageNavigator.CanGoBack )
					{
					//-----------------------------------------------------------------------------------------------------------
					this.GoBack ();
		...
		private void NavigateTo ( bool AllowCache, object Source )
			...
			if ( Source is SectionType )
				...
				if ( (SectionType)Source == SectionType.Bookmarks )
					RestService.GetBookmarks    ( this.NavigateOnComplete             );
				else
					RestService.GetArticlesList ( this.NavigateOnComplete, AllowCache );
				//---------------------------------------------------------------------------------------------------------------
				}
			//-------------------------------------------------------------------------------------------------------------------
			...
			//-------------------------------------------------------------------------------------------------------------------
				...
				RestService.GetArticle ( NavigateOnComplete, AllowCache, (Article)Source );
				//---------------------------------------------------------------------------------------------------------------
				}
			//-------------------------------------------------------------------------------------------------------------------
			...
			//-------------------------------------------------------------------------------------------------------------------
				...
				{
				//---------------------------------------------------------------------------------------------------------------
				this.CurrentSection = SectionType.Search;

				this.SetTitle ( this.CurrentSection );

				RestService.SearchArticles ( NavigateOnComplete, AllowCache, (string)Source );
				//---------------------------------------------------------------------------------------------------------------
				}
			//-------------------------------------------------------------------------------------------------------------------
			}
			...
			if ( QueryString.ContainsKey ( "Method" ) )
				...
				switch ( QueryString["Method"] )
					...
					case "GetPreviousArticles" :
						...
						if ( QueryString.ContainsKey ( "Pattern" ) )
							Pattern = QueryString["Pattern"];
						//-------------------------------------------------------------------------------------------------------
						...
						if ( ! string.IsNullOrEmpty ( Pattern ) )
							{
							//---------------------------------------------------------------------------------------------------
							RestService.SearchPreviousArticles ( (S, A) =>
								{
								//-----------------------------------------------------------------------------------------------
						...
						//-------------------------------------------------------------------------------------------------------
							...
							{
							//---------------------------------------------------------------------------------------------------
							RestService.GetPreviousArticlesList ( (S, A) =>
								{
								//-----------------------------------------------------------------------------------------------
								if ( A.Result == RestRequestResult.Success && 
															 ! string.IsNullOrEmpty ( A.Content ) )
		...
		//***********************************************************************************************************************
		#region // Section des Procédures de Gestion de la Recherche
		//-----------------------------------------------------------------------------------------------------------------------
		...
		private void SearchControl_KeyDown ( object sender, KeyEventArgs Args )
			...
			if ( Args.Key == Key.Enter )
				...
				this.Dispatcher.Async ( () => 
					{
					//-----------------------------------------------------------------------------------------------------------
					this.Navigate ( false, this.SearchControl.Text );
				...
				//---------------------------------------------------------------------------------------------------------------
					...
					if ( ((SectionType)CurrentObject) == SectionType.All )
						...
						this.ApplicationBar.SetButtons ( Mode                      ,
							                             ...
							                             this.OptionsAppBarButton  ,
							                             this.RefreshAppBarButton  );
						//-------------------------------------------------------------------------------------------------------
						}
					//-----------------------------------------------------------------------------------------------------------
					else if ( ((SectionType)CurrentObject) == SectionType.Search )
						{
						//-------------------------------------------------------------------------------------------------------
						this.ApplicationBar.SetButtons ( Mode                      ,
		...
		//***********************************************************************************************************************
		/// <summary>
		/// Se produit lors d'un click sur le bouton "mes favoris".
		/// </summary>
		/// <param name="Sender">Objet source de l'appel</param>
		/// <param name="Args"><b>EventArgs</b> qui contient les données d'événement.</param>
		//-----------------------------------------------------------------------------------------------------------------------
		private void OnBookmarksButtonClick ( object Sender, RoutedEventArgs Args )
			{
			//-------------------------------------------------------------------------------------------------------------------
			this.Navigate ( true, SectionType.Bookmarks );
			//-------------------------------------------------------------------------------------------------------------------
			}
		//***********************************************************************************************************************
		...
		private void OnSearchButtonClick ( object Sender, RoutedEventArgs Args )
			...
			this.Dispatcher.Async ( () =>  { this.SearchControl.Focus (); }, 350 );
			//-------------------------------------------------------------------------------------------------------------------
			}
		//***********************************************************************************************************************
		...
		/// <param name="Args"><b>EventArgs</b> qui contient les données d'événement.</param>
		//-----------------------------------------------------------------------------------------------------------------------
		private void OnOptionsButtonClick ( object Sender, EventArgs Args )
			{
			//-------------------------------------------------------------------------------------------------------------------
		...
		private void OnShareButtonClick ( object Sender, EventArgs Args )
			...
			if ( Item != null )
				...
				if ( string.IsNullOrEmpty ( Title ) )
					Title = string.Format ( "Article n°{0} - ZDNet.com", Item.ArticleID );
				//---------------------------------------------------------------------------------------------------------------
